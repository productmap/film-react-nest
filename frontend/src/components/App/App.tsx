  ...
  return (
    <>
      <Layout isLocked={!!state.modal}>
        <Header
          counter={state.basket.length}
          onClick={handlers.handleOpenBasket}
        />
        {data.preview && (
          <FilmPreview {...data.preview} onClick={handlers.handleOpenFilm} />
        )}
        <FilmsGallery
          items={state.films}
      ...
      {state.modal && data.preview && (
        ...
        >
          ...
          {state.modal === "places" && data.session && (
            <SelectPlaces
          ...
          {state.modal === "basket" && (
            <Basket items={data.basket} onDelete={handlers.removeTicket} />
          )}
